(() => {
  document.addEventListener('DOMContentLoaded', function () {
    ...
    let audioIcon = document.querySelectorAll('.blockSub_audio');
    let timeLesson = '';
    let nextOpenDate = '';
    let audioSvg = '';
    ...
    slideBefor.forEach((befor) => {
      ...
      audioIcon.forEach((audio) => {
        if (audio.closest('.blockSub-slide_wrapper-img').querySelector('.blockSub-slide_before') === befor) {
          if (audio.innerText.trim().length > 0) {
            audioSvg = `<div class="blockSub-slide_before_svgMusic">
