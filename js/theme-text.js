(() => {
  ...
  function changeContent(elem) {
    ...
    //1 вариант
    if (elem.text.length > 600) {
      let firstText, lastText;

      //делим текст примерно пополам и рубим его по пробелу
      for (let i = Math.floor(elem.text.length / 2); i > 0; i--) {
        if (elem.text[i] === ' ') {
          firstText = elem.text.substring(0, i);
          lastText = elem.text.substring(i + 1)
          break;
        }
      }
    ...
    document.querySelector('.player_title_text').innerText = elem.audio;
  ...
  $.getJSON("processing",
    function (data) {
      ...
      //форирование объекта с данными из полученных
