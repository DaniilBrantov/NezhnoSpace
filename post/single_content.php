...
<?php
    ...
    if(!$post_id){
        ...
        $liked_post = '';
        // if ($cat_slug === "daily-practices" || $cat_slug === "recommendations") {
        //     $categoryId = ($cat_slug === "daily-practices") ? 45 : 46;
        //     $posts = $subscription->filterPosts($categoryId);
            
        //     foreach ($posts as &$liked_post) {
        ...
        // $post_data =$subscription->getSubscriptionLesson($post_id);
    ...
    if ($src && is_array($src) && !empty($src)) {
        $src = $src[0];
    } else {
        // Если изображение отсутствует, установите значение по умолчанию или обработайте ошибку.
        $src = 'Путь_к_изображению_по_умолчанию.jpg';
    }
?>
...
if($subscription->getFilterCat($post_id, "recommendations")){ 
    ...
    ?>
<div class='single_button-reaction' data-post-id="<?php echo $post_id; ?>"
...
<?php
    ...
    if (is_array($month_theme) && !empty($month_theme)) {
?>
